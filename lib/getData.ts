...
export const getNewsBySubcategory = (subcat: string): NewsItem[] =>
  (news as NewsItem[]).filter(n => n.subcategory.toLowerCase() === subcat.toLowerCase())
...
export const getLatestNews = (limit?: number): NewsItem[] => {
  const sorted = (news as NewsItem[]).sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
...
export const getArticleBySubcategory = (subcat: string): ArticleItem[] =>
  (articles as ArticleItem[]).filter(a => a.subcategory.toLowerCase() === subcat.toLowerCase())
...
export const getLatestArticles = (limit?: number): ArticleItem[] => {
  const sorted = (articles as ArticleItem[]).sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
  return limit ? sorted.slice(0, limit) : sorted
}

// Utility functions for both news and articles
export const getItemsByAuthor = (author: string): (NewsItem | ArticleItem)[] => {
  const newsByAuthor = (news as NewsItem[]).filter(n => n.author.toLowerCase().includes(author.toLowerCase()))
  const articlesByAuthor = (articles as ArticleItem[]).filter(a => a.author.toLowerCase().includes(author.toLowerCase()))
  return [...newsByAuthor, ...articlesByAuthor]
}

export const searchItems = (query: string): (NewsItem | ArticleItem)[] => {
  const searchLower = query.toLowerCase()
  const newsResults = (news as NewsItem[]).filter(n => 
    n.title.toLowerCase().includes(searchLower) ||
    n.content.toLowerCase().includes(searchLower) ||
    n.excerpt.toLowerCase().includes(searchLower)
  )
  const articleResults = (articles as ArticleItem[]).filter(a => 
