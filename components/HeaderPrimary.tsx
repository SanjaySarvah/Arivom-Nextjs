...
import {
  FaFacebookF,FaInstagram,FaTiktok,FaPinterestP,FaAmazon,FaPause,FaPlay,FaUser,FaChevronDown,FaUserPlus,
} from "react-icons/fa";
...
const HeaderPrimary: React.FC = () => {
  ...
  const increaseSpeed = () => {
    setMarqueeSpeed(prev => Math.max(10, prev - 10));
  };

  const decreaseSpeed = () => {
    setMarqueeSpeed(prev => Math.min(100, prev + 10)); 
  };
  ...
  return (
    <>
      ...
      <div
        className={`hidden xl:block transition-all duration-300 hidden xl:block ${isScrolled ? "py-2 shadow-sm" : "py-3"
          }`}
      >
        <div className="max-w-[1320px] px-[15px] mx-auto flex items-center justify-between">
      ...
      >
        ...
        <div
          ...
          <div className="flex items-center justify-between p-5 border-b-2 border-[#2ecc71]">
            <button onClick={toggleMenu} className="p-1 hover:bg-emerald-50 rounded transition-colors"
            >
